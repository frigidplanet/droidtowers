...
public class CloudGameSave extends TowerGameServiceObject {
	...
	public GameSave getGameSave() {
		GameSave gameSave = GameSaveFactory.readFile(new ByteArrayInputStream(
				getBlob().getBytes()), "cloudGameSave_" + hashCode());
	...
	public List<String> getNeighbors() {
		...
		if (neighborGameSaves != null) {
			for (FriendCloudGameSave gameSave : neighborGameSaves.getObjects()) {
				neighborUris.add(URI.create(gameSave.getResourceUri())
						.getPath());
			}
		}
	...
	public GameSave getGameSaveMetadata() {
		return GameSaveFactory.readMetadata(new ByteArrayInputStream(getBlob()
				.getBytes()));
	}
	...
	public String getBlob() {
		if (!upgradedGameSave) {
			...
			try {
				blob = getObjectMapper()
						.writeValueAsString(
								GameSaveFactory.upgradeGameSave(
	...
	public boolean hasNeighbors() {
		return ((neighbors != null) && !neighbors.isEmpty())
				|| ((neighborGameSaves != null) && !neighborGameSaves.isEmpty());
	}
