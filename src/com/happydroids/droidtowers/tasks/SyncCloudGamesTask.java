...
public class SyncCloudGamesTask extends BackgroundTask {
	protected static final String TAG = SyncCloudGamesTask.class
			.getSimpleName();
	...
	private void syncCloudGameSaves(CloudGameSaveCollection gameSaves,
			final FileHandle storage, final FileHandle[] files) {
		Set<String> towersProcessed = Sets.newHashSet();
		if (files != null && files.length > 0) {
			for (FileHandle file : files) {
				try {
					GameSave towerData = GameSaveFactory.readMetadata(file
							.read());
					for (CloudGameSave cloudGameSave : gameSaves.getObjects()) {
						if (towerData.getCloudSaveUri() != null
								&& towerData.getCloudSaveUri().equals(
										cloudGameSave.getResourceUri())) {
							if (towerData.getFileGeneration() < cloudGameSave
									.getFileGeneration()) {
		...
		for (CloudGameSave cloudGameSave : gameSaves.getObjects()) {
			if (!towersProcessed.contains(cloudGameSave.getResourceUri())) {
				try {
					Gdx.app.debug(TAG,
							"Could not find: " + cloudGameSave.getResourceUri()
									+ " on disk!");
					GameSave gameSave = cloudGameSave.getGameSave();
					GameSaveFactory.save(gameSave,
							storage.child(gameSave.getBaseFilename()));
					storage.child(gameSave.getBaseFilename() + ".png")
							.writeBytes(cloudGameSave.getImage(), false);
				} catch (Exception ignored) {
				}
			}
		}
