...
public class CommercialSpace extends Room {
	...
	public void updateJobs() {
		...
		if (isConnectedToTransport()) {
			...
			if (Player.instance().getTotalPopulation() > commercialType
					.getPopulationRequired()) {
				int jobsProvided = commercialType.getJobsProvided();
				if (jobsProvided > 0) {
					jobsFilled = Random.randomInt(jobsProvided / 2,
							jobsProvided);
				}
			}
		}
	...
	public float getNoiseLevel() {
		if (jobsFilled > 0) {
			return gridObjectType.getNoiseLevel()
					* ((float) jobsFilled / ((CommercialType) gridObjectType)
							.getJobsProvided());
		}
	...
	public int getCoinsEarned() {
		if (jobsFilled > 0 && isConnectedToTransport()) {
			return (int) Math.ceil(gridObjectType.getCoinsEarned()
					* getDesirability() + getUpkeepCost()
					+ gridObjectType.getCoinsEarned() * 0.06125f
					* getNumVisitors());
		}
	...
	protected void checkDecals() {
		...
		if (unlockedJanitors && unlockedMaids && getDirtLevel() >= 0.95f
				&& jobsFilled > 0) {
