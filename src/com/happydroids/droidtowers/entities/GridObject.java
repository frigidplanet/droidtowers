...
public abstract class GridObject {
	...
	// The number of visitors since the object was last cleaned
	protected int visitorsSinceClean;
	...
	public void recordVisitor(Avatar avatar) {
		...
		if (avatar instanceof Janitor || avatar instanceof Maid) {
			...
			if (provides(janitor.servicesTheseProviderTypes)) {
				...
				averageNumVisitors.record(visitorsSinceClean);
				resetVisitorSinceClean();
			}
		}
	}

	/**
	 * Updates the visitor counts on the objects
	 */
	private void addVisitor() {
		this.visitorsSinceClean++;
		this.totalVisitors++;
	}

	/**
	 * Resets the visitor count since last cleaning
	 */
	private void resetVisitorSinceClean() {
		this.visitorsSinceClean = 0;
	}
