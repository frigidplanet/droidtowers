...
public class ServiceRoomType extends CommercialType {
	...
	public boolean canBeAt(GridObject gridObject) {
		if (gridObject.provides(ProviderType.PARKING)
			...
			if (gridObject.provides(ProviderType.PARKING_SPACE)) {
				...
				if (checkPointForProviderType(gridObject,
						objectGridPoint.sub(1, 0), ProviderType.PARKING)) {
					return true;
				}
				// check single point to the right
				if (checkPointForProviderType(gridObject,
						objectGridPoint.add(2, 0), ProviderType.PARKING)) {
					return true;
				}
				...
				if (yPos == TowerConsts.LOBBY_FLOOR) {
					...
					GridPoint objectGridPoint = gridObject.getPosition().cpy();
					// check below
					Array<GridObject> gridObjects = gridObject
							.getGameGrid()
							.positionCache()
							.getObjectsAt(objectGridPoint.sub(0, 1),
									gridObject.getSize(), gridObject);
					for (GridObject object : gridObjects) {
						if (object.provides(ProviderType.PARKING_RAMP)
								&& object.getPosition().x == gridObject
										.getPosition().x) {
							return true;
						}
					}
					...
					gridObjects = gridObject
							.getGameGrid()
							.positionCache()
							.getObjectsAt(objectGridPoint.add(0, 2),
									gridObject.getSize(), gridObject);
					for (GridObject object : gridObjects) {
						if (object.provides(ProviderType.PARKING_RAMP)
								&& object.getPosition().x == gridObject
	...
	private boolean checkPointForProviderType(GridObject gridObject,
			GridPoint gridPoint, final ProviderType providerType) {
