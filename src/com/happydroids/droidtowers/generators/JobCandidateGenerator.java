...
public class JobCandidateGenerator {
	public static List<JobCandidate> generate(int numToGenerate) {
		...
		for (int i = 0; i < numToGenerate; i++) {
			...
			candidate.setGender(MathUtils.random(10) > 5 ? Gender.FEMALE
					: Gender.MALE);
	...
	private static String generateUniqueName(List<JobCandidate> candidates,
		...
		while (hasDupes) {
			String name = gender.equals(Gender.FEMALE) ? NameGenerator
					.randomFemaleName() : NameGenerator.randomMaleName();
			hasDupes = false;
