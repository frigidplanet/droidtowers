...
public class SellTool extends ToolBase {
	...
	public boolean touchDown(float x, float y, int pointer, int button) {
		...
		Array<GridObject> gridObjects = gameGrid.positionCache().getObjectsAt(
				gridPointAtFinger, TowerConsts.SINGLE_POINT);
		...
		if (gridObjects != null && gridObjects.size > 0) {
			...
			if (objectToSell.getPosition().y >= TowerConsts.LOBBY_FLOOR
					&& !checkAboveOrBelow(objectToSell, objectToSell
							.getPosition().cpy().add(0, 1))) {
				HeadsUpDisplay
						.showToast("You cannot sell this, something is above it.");
			} else if (objectToSell.getPosition().y < TowerConsts.LOBBY_FLOOR
					&& !checkAboveOrBelow(objectToSell, objectToSell
							.getPosition().cpy().sub(0, 1))) {
				HeadsUpDisplay
						.showToast("You cannot sell this, something is below it.");
			} else {
				final int sellPrice = (int) (objectToSell.getGridObjectType()
						.getCoins() * 0.5);
				new SellGridObjectConfirmationDialog(gameGrid, objectToSell)
						.show();
			}
	...
	private boolean checkAboveOrBelow(GridObject objectToSell,
			final GridPoint gridPointAbove) {
		if (objectToSell instanceof Transit) {
			return true;
		}

		Array<GridObject> objectsAbove = objectToSell
				.getGameGrid()
				.positionCache()
				.getObjectsAt(gridPointAbove, objectToSell.getSize(),
						objectToSell);
