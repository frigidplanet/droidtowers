...
public class GridObjectDesigner extends TowerWindow {
	...
	public GridObjectDesigner(Stage stage) {
		...
		ScrollPane.ScrollPaneStyle paneStyle = new ScrollPane.ScrollPaneStyle(
				scrollPane.getStyle());
		paneStyle.background = TowerAssetManager.ninePatchDrawable(
				TowerAssetManager.WHITE_SWATCH, Color.LIGHT_GRAY);
		...
		getContent().addListener(new ActorGestureListener() {
			...
			public void zoom(InputEvent event, float initialDistance,
					float distance) {
				float zoom = MathUtils.clamp(initialZoom * distance
						/ initialDistance, 1f, 1.5f);
	...
	private void addAtlasItemsToSidebar(Table sidebar,
		...
		for (final TextureAtlas.AtlasRegion region : atlas.getRegions()) {
			sidebar.row();
			final Image image = new Image(new TextureRegionDrawable(region),
					Scaling.fit);
			...
			image.addListener(new InputListener() {
				...
				public boolean touchDown(InputEvent event, float x, float y,
					...
					selectedItem.addAction(Actions.sequence(
							Actions.scaleTo(1.25f, 1.25f, 0.15f),
							Actions.scaleTo(1f, 1f, 0.15f)));
					selectedItem.setPosition(event.getStageX() - x,
							event.getStageY() - y);
					...
					inputProcessor.setOriginalPosition(new Vector2(event
							.getStageX(), event.getStageY()));
