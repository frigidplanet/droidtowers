...
public class ViewNeighborSplashScene extends SplashScene {
	...
	private class FetchNeighborsList extends BackgroundTask {
		...
		protected void execute() throws Exception {
			...
			if (TowerGameService.instance().isAuthenticated()) {
				CloudGameSave cloudGameSave = playerGameState
						.getCloudGameSave();
				...
				FriendCloudGameSaveCollection neighborGameSaves = cloudGameSave
						.getNeighborGameSaves();
				if (!neighborGameSaves.isEmpty()) {
					int progressPerFetch = 66 / neighborGameSaves.size();
					int progressSoFar = 33;
					for (FriendCloudGameSave friendGameSave : neighborGameSaves
							.getObjects()) {
		...
		public synchronized void afterExecute() {
			...
			if (fetchWasSuccessful) {
				...
				SceneManager
						.pushScene(ViewNeighborScene.class, playerGameState);
			} else {
	...
	private void displayConnectionErrorDialog() {
		new Dialog()
				...
				.setMessage(
						"Sorry, but we were not able to fetch your neighborhood.\n\nPlease check your internet connection and try again.")
