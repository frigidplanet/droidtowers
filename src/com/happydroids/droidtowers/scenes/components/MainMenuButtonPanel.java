...
public class MainMenuButtonPanel extends Table {
	...
	public MainMenuButtonPanel() {
		...
		if (TowerConsts.ENABLE_HAPPYDROIDS_CONNECT) {
			...
			TowerGameService.instance().afterDeviceIdentification(
					new Runnable() {
						public void run() {
							Gdx.app.debug(TAG,
									"After auth, close/show connect button.");
							if (!TowerGameService.instance().isAuthenticated()) {
								connectToHappyDroids.setVisible(true);
								connectToHappyDroids
										.addListener(new VibrateClickListener() {
											...
											public void onClick(
													...
													float y) {
												SceneManager
														.pushScene(HappyDroidConnect.class);
											}
										});
								connectToHappyDroids.addAction(Actions
										.fadeIn(0.25f));
							}
						}
					});
