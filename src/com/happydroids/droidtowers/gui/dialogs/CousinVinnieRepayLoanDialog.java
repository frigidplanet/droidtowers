...
public class CousinVinnieRepayLoanDialog extends Dialog {
	public CousinVinnieRepayLoanDialog(final Room gridObject) {
		...
		TextureRegionDrawable cousinVinnieTexture = TowerAssetManager
				.drawableFromAtlas("droid-cousin-vinnie", "hud/menus.txt");
		...
		c.row();
		String loanAmount = TowerConsts.CURRENCY_SYMBOL
				+ StringUtils.formatNumber(gridObject
						.getAmountLoanedFromCousinVinnie());
		final float repayAmount = gridObject.getAmountLoanedFromCousinVinnie() * 1.25f;
		String repayAmountStr = TowerConsts.CURRENCY_SYMBOL
				+ StringUtils.formatNumber(repayAmount);
		c.add(FontManager.Roboto18.makeLabel("Hey Cousin,\n" + "\n"
				+ "So you want to pay me off eh?.\n" + "\n" + "I loaned you "
				+ loanAmount + " at 25% interest,\n" + " so I think "
				+ repayAmountStr + " sounds about right.\n" + "\n"
				+ "Regards,\n" + "Vinnie "));
			...
			public void onClick(InputEvent event, float x, float y) {
				if (Player.instance().getCoins() < repayAmount) {
					new Dialog().setMessage(
							"You do not have enough money to pay Vinnie back.")
							.show();
				} else {
