...
public class InviteNonPlayerFriendWindow extends TowerWindow {
	...
	public InviteNonPlayerFriendWindow(NonPlayerFriend profile) {
		super("Invite " + profile.getFriendName() + " to Droid Towers!",
				SceneManager.activeScene().getStage());
		...
		add(
				FontManager.Roboto18.makeLabel("Message that will be sent to: "
						+ profile.getFriendName())).expandX();
		row().fillX();
		add(FontManager.Roboto18.makeLabel(inviteText)).height(
				Display.devicePixel(250)).expandX();
	}
}
