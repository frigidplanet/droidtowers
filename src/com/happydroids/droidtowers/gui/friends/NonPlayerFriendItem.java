...
public class NonPlayerFriendItem extends PlayerFriendItem {
	...
	protected TextButton makeActionButton() {
		TextButton inviteButton = FontManager.Roboto18
				.makeTextButton("Invite to Play");
		inviteButton.addListener(new VibrateClickListener() {
			...
			public void onClick(InputEvent event, float x, float y) {
				...
				Platform.getBrowserUtil().launchWebBrowser(
						HappyDroidConsts.HAPPYDROIDS_URI + "/login?token="
								+ TowerGameService.instance().getSessionToken()
								+ "&next=/friend/" + profile.getId()
								+ "/invite/");
			}
		});
		return inviteButton;
