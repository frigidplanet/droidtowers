...
public class PurchaseDroidTowersUnlimitedPrompt extends ScrollableTowerWindow {
	...
	public PurchaseDroidTowersUnlimitedPrompt() {
		super("Purchase Droid Towers: Unlimited", DroidTowersGame
				.getRootUiStage());
		...
		addLabel("All these features will be enabled:",
				FontManager.RobotoBold18).expandX().padTop(
				Display.devicePixel(8));
		int indentPixels = Display.devicePixel(64);
		addLabel("* Towers taller up to 250 floors", FontManager.RobotoBold18)
				.padLeft(indentPixels);
		addLabel("* Unlock the Sky Lobby:", FontManager.RobotoBold18).padLeft(
				indentPixels);
		row();
		add(
				new Image(RoomTypeFactory.findTypeById("SKY-LOBBY")
						.getTextureRegion(0))).center();
		addLabel("* Unlock Land expansions", FontManager.RobotoBold18).padLeft(
				indentPixels);
		addLabel("* Earn income 50% faster", FontManager.RobotoBold18).padLeft(
				indentPixels);
		addLabel("* Feeds the developers for less than $0.01 USD per day!",
				FontManager.RobotoBold18).padLeft(indentPixels);
		addLabel("* FREE content updates!", FontManager.Roboto24, Color.CYAN)
				.padLeft(indentPixels);

		shoveContentUp();

		TextButton purchaseButton = FontManager.Roboto32.makeTextButton(
				"Purchase for: $0.99", Color.GREEN);
		...
		TextButton dismissButton = FontManager.Roboto18
				.makeTextButton("No Thanks");
		...
		new Dialog()
				...
				.addButton("Purchase for $1.99", new OnClickCallback() {
					...
					public void onClick(Dialog dialog) {
						dialog.dismiss();
						Platform.getPurchaseManager()
								.requestPurchaseForDiscountedOffer();
					}
