...
public class ConnectToHappyDroidsWindow extends TowerWindow {
	private static final String TAG = ConnectToHappyDroidsWindow.class
			.getSimpleName();
	...
	public ConnectToHappyDroidsWindow(Stage stage) {
		...
		openBrowserButton = FontManager.RobotoBold18
				.makeTextButton("Open my web browser");
		openBrowserButton.setVisible(false);

		sessionStatus = FontManager.Roboto24
				.makeLabel("Waiting for You to login...");
	...
	private class TemporaryTokenApiRunnable extends ApiRunnable {
		@Override
		public void onError(HttpResponse response, int statusCode,
				HappyDroidServiceObject object) {
			sessionStatus.setText(String
					.format("Login failed: %d!", statusCode));

			new Dialog(DroidTowersGame.getRootUiStage())
					.setTitle("Connection Error")
					.setMessage(
							"Could not contact happydroids.com, please your internet connection and try again.\n\nERROR:ETFAIL2FONHOME")
		...
		public void onSuccess(HttpResponse response,
			...
			openBrowserButton.addListener(new VibrateClickListener() {
				...
				public void onClick(InputEvent event, float x, float y) {
					Gdx.app.postRunnable(new LaunchBrowserAfterDelay(token
							.getClickableUri(), 1.5f));
				}
			});
		...
		private class AccessTokenCheckStateTask extends PeriodicBackgroundTask {
			...
			public boolean update() {
				...
				try {
					...
					Gdx.app.error(TAG, "Error validating the temporary token.",
							e);
				}
			...
			public synchronized void afterExecute() {
				if (token != null && token.hasSessionToken()) {
					...
					new Dialog()
							.setTitle("Connect to happydroids.com")
							.setMessage(
									"Sorry, but we were unable to connect to happydroids.com.\n\nPlease try again later.")
							.addButton("Dismiss", new OnClickCallback() {
								@Override
								public void onClick(Dialog dialog) {
									dialog.dismiss();
