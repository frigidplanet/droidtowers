...
public class RegisterDeviceTask extends BackgroundTask {
	protected static final String TAG = RegisterDeviceTask.class
			.getSimpleName();
	...
	public synchronized void afterExecute() {
		...
		if (device != null) {
			Gdx.app.debug(TAG, "Authentication finished, state: "
			...
			if (device.payment != null) {
				if (!device.payment.wasRefunded()) {
					Platform.getPurchaseManager().purchaseItem(
							device.payment.getItemId(),
							device.payment.getSerial());
				} else {
