...
public class AESObfuscator {
	...
	public AESObfuscator(byte[] salt, String password) {
		try {
			...
			mEncryptor.init(Cipher.ENCRYPT_MODE, secret,
					new IvParameterSpec(IV));
			mDecryptor = Cipher.getInstance(CIPHER_ALGORITHM);
			mDecryptor.init(Cipher.DECRYPT_MODE, secret,
					new IvParameterSpec(IV));
	...
	public String obfuscate(String original) {
		...
		try {
			...
			return Base64.encode(mEncryptor.doFinal((header + original)
					.getBytes(UTF8)));
	...
	public String unobfuscate(String obfuscated) throws ValidationException {
		...
		try {
			String result = new String(mDecryptor.doFinal(Base64
					.decode(obfuscated)), UTF8);
