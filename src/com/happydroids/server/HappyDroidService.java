...
public class HappyDroidService {
	...
	private final Set<Runnable> withNetworkConnectionRunnables = Sets
			.newHashSet();
	...
	public HttpResponse makePutRequest(String uri, Object objectForServer) {
		...
		try {
			...
			if (objectForServer != null) {
				...
				String apiObjectAsString = mapper
						.writeValueAsString(objectForServer);
	...
	public HttpResponse makePostRequest(String uri, Object objectForServer) {
		...
		try {
			...
			if (objectForServer != null) {
				ObjectMapper mapper = getObjectMapper();
				Gdx.app.debug(TAG,
						"JSON: " + mapper.writeValueAsString(objectForServer));
				StringEntity entity = new StringEntity(
						mapper.writeValueAsString(objectForServer), "UTF-8");
				entity.setContentType("application/x-www-form-urlencoded");
				request.setEntity(entity);
			}
	...
	public HttpResponse makeGetRequest(String url,
			HashMap<String, String> objectForServer) {
		return makeGetRequest(url, objectForServer, false, -1);
	}

	public HttpResponse makeGetRequest(String urlString,
			HashMap<String, String> queryParams, boolean enableCache,
			int cacheMaxAge) {
		try {
			URI uri = new URI(urlString);

			if (queryParams != null && queryParams.size() > 0) {
				List<NameValuePair> pairs = Lists
						.newArrayListWithCapacity(queryParams.size());
				for (String paramName : queryParams.keySet()) {
					pairs.add(new BasicNameValuePair(paramName, queryParams
							.get(paramName)));
				}
			...
			CacheResponseStatus responseStatus = (CacheResponseStatus) context
					.getAttribute(CACHE_RESPONSE_STATUS);

			if (responseStatus == CACHE_HIT) {
				Gdx.app.debug(TAG,
						"CACHED: GET " + uri + ", " + response.getStatusLine());
			} else {
				Gdx.app.debug(TAG,
						"RES: GET " + uri + ", " + response.getStatusLine());
			}
