...
public abstract class HappyDroidServiceCollection<CollectionType extends HappyDroidServiceObject> {
	...
	public void fetch(
			final ApiCollectionRunnable<HappyDroidServiceCollection<CollectionType>> apiRunnable) {
		fetchBlocking(apiRunnable);
	}

	private void copyValuesFromResponse(HttpResponse response)
			throws IOException {
		HappyDroidServiceCollection collection = HappyDroidService
				.instance()
				.getObjectMapper()
				.readValue(
	...
	public void fetchBlocking(
			ApiCollectionRunnable<HappyDroidServiceCollection<CollectionType>> apiRunnable) {
		fetching = true;
		HttpResponse response = HappyDroidService.instance().makeGetRequest(
				getBaseResourceUri(), currentFilters, isCachingAllowed(),
				getCacheMaxAge());
		fetching = false;
		if (response != null && response.getStatusLine() != null
				&& response.getStatusLine().getStatusCode() == 200) {
			ObjectMapper objectMapper = HappyDroidService.instance()
					.getObjectMapper();
			try {
	...
	public HappyDroidServiceCollection<CollectionType> filterBy(
			final String fieldName, final String filterValue) {
	...
	public HappyDroidServiceCollection<CollectionType> filterBy(
			final String fieldName, long filterValue) {
	...
	public void add(CollectionType object) {
		for (CollectionType collectionType : objects) {
			if (collectionType.getResourceUri() != null
					&& collectionType.getResourceUri().equals(
